...
Q_LOGGING_CATEGORY(QtJsonSerializer::logCbor,
                   "qt.jsonserializer.serializer.cbor")
...
void CborSerializer::setTypeTag(int metaTypeId, QCborTag tag) {
  Q_D(CborSerializer);
  Q_ASSERT_X(metaTypeId != QMetaType::UnknownType, Q_FUNC_INFO,
             "You cannot assign a tag to QMetaType::UnknownType");
  ...
  if (tag == TypeConverter::NoTag) {
    ...
    qCDebug(logCbor) << "Removed Type-Tag for metaTypeId"
...
QCborTag CborSerializer::typeTag(int metaTypeId) const {
  ...
  if (tag != TypeConverter::NoTag) {
    qCDebug(logCbor) << "Found Type-Tag for metaTypeId"
                     << QMetaTypeName(metaTypeId) << "as" << tag;
  } else
    qCDebug(logCbor) << "No Type-Tag found for metaTypeId"
                     << QMetaTypeName(metaTypeId);
  return tag;
}
...
void CborSerializer::serializeTo(QIODevice *device, const QVariant &data,
                                 QCborValue::EncodingOptions options) const {
...
QByteArray
CborSerializer::serializeTo(const QVariant &data,
                            QCborValue::EncodingOptions options) const {
  return serializeVariant(data.userType(), data).toCbor(options);
}

QVariant CborSerializer::deserialize(const QCborValue &cbor, int metaTypeId,
                                     QObject *parent) const {
  return deserializeVariant(metaTypeId, cbor, parent);
}

QVariant CborSerializer::deserializeFrom(QIODevice *device, int metaTypeId,
                                         QObject *parent) const {
  ...
  if (const auto error = reader.lastError(); error.c != QCborError::NoError)
    throw DeserializationException("Failed to read file as CBOR with error: " +
                                   error.toString().toUtf8());
  return deserializeVariant(metaTypeId, cbor, parent);
}

QVariant CborSerializer::deserializeFrom(const QByteArray &data, int metaTypeId,
                                         QObject *parent) const {
  QCborParserError error;
  const auto cbor = QCborValue::fromCbor(data, &error);
  if (error.error.c != QCborError::NoError)
    throw DeserializationException("Failed to read file as CBOR with error: " +
                                   error.error.toString().toUtf8());
  return deserializeVariant(metaTypeId, cbor, parent);
}

std::variant<QCborValue, QJsonValue>
CborSerializer::serializeGeneric(const QVariant &value) const {
  return serialize(value);
}

QVariant CborSerializer::deserializeGeneric(
    const std::variant<QCborValue, QJsonValue> &value, int metaTypeId,
    QObject *parent) const {
...
bool CborSerializer::jsonMode() const { return false; }

QList<int> CborSerializer::typesForTag(QCborTag tag) const {
  Q_D(const CborSerializer);
  QReadLocker lock{&d->typeTagsLock};
  const auto keys = d->typeTags.keys(tag);
  qCDebug(logCbor) << "Found metaTypeIds for tag" << tag << "as" << keys;
  return keys;
}
...
template <typename TInt> bool testOverflow(const QByteArray &data) {
  return (data.size() == static_cast<int>(sizeof(TInt))) &&
         (data[0] & 0x80) != 0;
}

template <typename TInt, bool Invert = false> TInt extract(QByteArray data) {
...
QVariant
CborSerializerPrivate::deserializeCborValue(int propertyType,
...
QVariant
CborSerializerPrivate::deserializePositiveBignum(const QByteArray &data) const {
  const auto dSize = static_cast<size_t>(data.size());
  if (dSize > static_cast<int>(sizeof(quint64))) {
    throw DeserializationException{
        "Unable to handle PositiveBignum tagged integers, bigger then " +
        QByteArray::number(static_cast<int>(sizeof(quint64))) + " bytes"};
  }
...
QVariant
CborSerializerPrivate::deserializeNegativeBignum(const QByteArray &data) const {
  const auto dSize = static_cast<size_t>(data.size());
  if (dSize > static_cast<int>(sizeof(qint64)) || testOverflow<qint64>(data)) {
    throw DeserializationException{
        "Unable to handle NegativeBignum tagged integers, bigger then " +
...
qreal CborSerializerPrivate::deserializeDecimal(const QCborArray &data) const {
  if (data.size() != 2)
    throw DeserializationException{
        "Decimal tagged types must be an array with exactly two elements"};
...
qreal CborSerializerPrivate::deserializeBigfloat(const QCborArray &data) const {
  if (data.size() != 2)
    throw DeserializationException{
        "Bigfloat tagged types must be an array with exactly two elements"};
  return std::ldexp(data[1].toInteger(), data[0].toInteger());
}

qreal CborSerializerPrivate::deserializeRationaleNumber(
    const QCborArray &data) const {
  if (data.size() != 2)
    throw DeserializationException{"RationaleNumber tagged types must be an "
