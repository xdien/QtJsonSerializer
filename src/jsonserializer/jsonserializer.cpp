...
void JsonSerializer::serializeTo(QIODevice *device, const QVariant &data,
  ...
  if (jData.isArray())
    ...
    throw SerializationException{
        "Only objects or arrays can be written to a device!"};
  device->write(doc.toJson(format));
}

QByteArray JsonSerializer::serializeTo(const QVariant &data,
                                       QJsonDocument::JsonFormat format) const {
  QBuffer buffer;
  if (!buffer.open(QIODevice::WriteOnly))
    throw SerializationException{
        "Failed to write to bytearray buffer with error: " +
        buffer.errorString().toUtf8()};
  serializeTo(&buffer, data, format);
  buffer.close();
  return buffer.data();
}

QVariant JsonSerializer::deserialize(const QJsonValue &json, int metaTypeId,
                                     QObject *parent) const {
  return deserializeVariant(metaTypeId, QCborValue::fromJsonValue(json),
                            parent);
}

QVariant JsonSerializer::deserializeFrom(QIODevice *device, int metaTypeId,
                                         QObject *parent) const {
  ...
  if (error.error != QJsonParseError::NoError)
    throw DeserializationException{"Failed to read file as JSON with error: " +
...
QVariant JsonSerializer::deserializeFrom(const QByteArray &data, int metaTypeId,
                                         QObject *parent) const {
  QBuffer buffer(const_cast<QByteArray *>(&data));
  if (!buffer.open(QIODevice::ReadOnly))
    throw DeserializationException{
        "Failed to read from bytearray buffer with error: " +
        buffer.errorString().toUtf8()};
...
JsonSerializer::serializeGeneric(const QVariant &value) const {
  return serialize(value);
}

QVariant JsonSerializer::deserializeGeneric(
...
void JsonSerializer::setByteArrayFormat(
    JsonSerializer::ByteArrayFormat byteArrayFormat) {
  Q_D(JsonSerializer);
...
bool JsonSerializer::jsonMode() const { return true; }
