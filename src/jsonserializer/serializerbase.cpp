...
namespace {
void qtJsonSerializerRegisterTypes() { QtJsonSerializer::registerTypes(); }
} // namespace
Q_COREAPP_STARTUP_FUNCTION(qtJsonSerializerRegisterTypes);
#endif

Q_LOGGING_CATEGORY(QtJsonSerializer::logSerializer,
                   "qt.jsonserializer.serializer")
Q_LOGGING_CATEGORY(QtJsonSerializer::logSerializerExtractor,
                   "qt.jsonserializer.serializer.extractor")
...
void SerializerBase::registerExtractor(
    int metaTypeId, const QSharedPointer<TypeExtractor> &extractor) {
  SerializerBasePrivate::extractors.add(metaTypeId, extractor);
  qCDebug(logSerializerExtractor)
      << "Added extractor for type:" << QMetaTypeName(metaTypeId);
}
...
void SerializerBase::addJsonTypeConverter(
    const QSharedPointer<TypeConverter> &converter) {
  Q_D(SerializerBase);
...
void SerializerBase::setPolymorphing(
    SerializerBase::Polymorphing polymorphing) {
  Q_D(SerializerBase);
...
void SerializerBase::setMultiMapMode(
    SerializerBase::MultiMapMode multiMapMode) {
  Q_D(SerializerBase);
...
QSharedPointer<const TypeExtractor>
SerializerBase::extractor(int metaTypeId) const {
  const auto extractor = SerializerBasePrivate::extractors.get(metaTypeId);
  if (extractor)
    qCDebug(logSerializerExtractor)
        << "Found extractor for type:" << QMetaTypeName(metaTypeId);
  else
    qCDebug(logSerializerExtractor)
        << "Unable to find extractor for type:" << QMetaTypeName(metaTypeId);
  return extractor;
}

QCborValue SerializerBase::serializeSubtype(const QMetaProperty &property,
                                            const QVariant &value) const {
  Q_D(const SerializerBase);
  ExceptionContext ctx(property);
  auto logGuard = qScopeGuard([]() {
    qCDebug(logSerializer) << QByteArray{">"}
                                  .repeated(ExceptionContext::currentDepth())
                                  .constData()
                           << "done";
  });
...
QCborValue SerializerBase::serializeSubtype(int propertyType,
                                            const QVariant &value,
                                            const QByteArray &traceHint) const {
  ...
  qCDebug(logSerializer)
      << QByteArray{">"}.repeated(ExceptionContext::currentDepth()).constData()
      << "Serializing subtype property" << traceHint << "of type"
      << QMetaTypeName(propertyType);
  return serializeVariant(propertyType, value);
}

QVariant SerializerBase::deserializeSubtype(const QMetaProperty &property,
  ...
  if (property.isEnumType()) {
    ...
    qCDebug(logSerializer) << QByteArray{">"}
                                  .repeated(ExceptionContext::currentDepth())
                                  .constData()
                           << "Deserializing subtype property"
                           << property.name() << "of enum type"
                           << QMetaTypeName(enumId);
    ...
    qCDebug(logSerializer) << QByteArray{">"}
                           ...
                           << QMetaTypeName(property.userType());
    return deserializeVariant(property.userType(), value, parent);
  }
}

QVariant SerializerBase::deserializeSubtype(int propertyType,
                                            const QCborValue &value,
                                            QObject *parent,
                                            const QByteArray &traceHint) const {
  ExceptionContext ctx(propertyType, traceHint);
  ...
  qCDebug(logSerializer)
      << QByteArray{">"}.repeated(ExceptionContext::currentDepth()).constData()
      << "Deserializing subtype property" << traceHint << "of type"
      << QMetaTypeName(propertyType);
  return deserializeVariant(propertyType, value, parent);
}

QCborValue SerializerBase::serializeVariant(int propertyType,
                                            const QVariant &value) const {
  Q_D(const SerializerBase);
  ...
  // second: check if an override tag is given, and if yes, override the normal
  // tag
...
QVariant SerializerBase::deserializeVariant(int propertyType,
                                            const QCborValue &value,
                                            QObject *parent,
                                            bool skipConversion) const {
  Q_D(const SerializerBase);
  // first: find a converter and convert the data to QVariant
  auto converter = d->findDeserConverter(
      propertyType, value.isTag() ? value.tag() : TypeConverter::NoTag,
  ...
  // second: if the type was given, enforce a conversion to that type (expect if
  // skipped)
  if (!skipConversion && propertyType != QMetaType::UnknownType) {
    ...
    if (allowConvert && variant.canConvert(propertyType) &&
        variant.convert(propertyType))
#else
    if (allowConvert && variant.canConvert(QMetaType(propertyType)) &&
        variant.convert(QMetaType(propertyType)))
#endif
      return variant;
...
SerializerBasePrivate::ThreadSafeStore<TypeExtractor>
    SerializerBasePrivate::extractors;
...
QSharedPointer<TypeConverter>
SerializerBasePrivate::findSerConverter(int propertyType) const {
  ...
  if (auto converter = serCache.get(propertyType); converter) {
    qCDebug(logSerializer) << "Found cached serialization converter"
                           << converter->name()
  ...
  for (const auto &converter : qAsConst(typeConverters.store)) {
    if (converter && converter->canConvert(propertyType)) {
      qCDebug(logSerializer)
          << "Found and cached serialization converter" << converter->name()
          << "for type:" << QMetaTypeName(propertyType);
  ...
  qCDebug(logSerializer)
      << "Unable to find serialization converte for type:"
      << QMetaTypeName(propertyType)
...
QSharedPointer<TypeConverter>
SerializerBasePrivate::findDeserConverter(int &propertyType, QCborTag tag,
                                          QCborValue::Type type) const {
  Q_Q(const SerializerBase);
  // first: update converters from factories
  updateConverterStore();

  // second: if no property type is given, try out any types associated with the
  // tag
  ...
  if (auto converter = deserCache.get(propertyType);
    ...
    qCDebug(logSerializer) << "Found cached deserialization converter"
                           << converter->name() << "for type"
                           << QMetaTypeName(propertyType) << LogTag{tag}
                           << "and CBOR-type" << type;
    return converter;
  }
  ...
  for (const auto &converter : qAsConst(typeConverters.store)) {
    if (converter) {
      ...
      qCDebug(logSerializer)
          << "Found and cached deserialization converter" << converter->name()
          << "for type" << QMetaTypeName(propertyType) << LogTag{tag}
          << "and CBOR-type" << type;
      return converter;
    }
  }
  ...
  if (guessConverter) {
    ...
    if (converter) {
      ...
      qCDebug(logSerializer)
          << "Found and cached deserialization converter" << converter->name()
          << "by guessing the data with CBOR-tag" << tag << "and CBOR-type"
          << type << "is of type" << QMetaTypeName(propertyType);
      return converter;
    }
  }
  ...
  if (throwWrongTag) {
    throw DeserializationException{
        QByteArray{"Found converter able to handle data of type "} +
        QMetaTypeName(propertyType) + ", but the given CBOR tag " +
  ...
  qCDebug(logSerializer)
      << "Unable to find deserialization converte for type"
      << QMetaTypeName(propertyType) << LogTag{tag} << "and CBOR-type" << type
...
void SerializerBasePrivate::updateConverterStore() const {
  ...
  if (typeConverterFactories.size() >
      typeConverters.factoryOffset.loadAcquire()) {
    ...
    for (auto i = typeConverters.factoryOffset.loadAcquire(); i < max; ++i) {
      ...
      if (converter) {
        ...
        deserCache.clear();
        qCDebug(logSerializer)
            << "Found and added new global converter:" << converter->name();
      }
    }
...
int SerializerBasePrivate::getEnumId(QMetaEnum metaEnum, bool ser) const {
  ...
  if (eTypeId == QMetaType::UnknownType) {
    if (ser)
      throw SerializationException{"Unable to determine typeid of meta enum " +
                                   eName};
    else
      throw DeserializationException{
          "Unable to determine typeid of meta enum " + eName};
  } else
    return eTypeId;
}

QCborValue SerializerBasePrivate::serializeValue(int propertyType,
                                                 const QVariant &value) const {
...
QVariant
SerializerBasePrivate::deserializeCborValue(int propertyType,
                                            const QCborValue &value) const {
  Q_Q(const SerializerBase);
  ...
  if (validationFlags.testFlag(ValidationFlag::StrictBasicTypes)) {
    ...
    case QMetaType::QString:
      expectedTags = {TypeConverter::NoTag,
    ...
    case QMetaType::QRegularExpression:
      if (!value.isRegularExpression()) {
        if (testValue.isString())
          expectedTags = {
              static_cast<QCborTag>(QCborKnownTags::RegularExpression)};
        else
          doThrow = true;
      }
      break;
    ...
    if (const auto mTag = q->typeTag(propertyType);
        mTag != TypeConverter::NoTag && mTag != value.tag())
      doThrow = true;
    else if (!expectedTags.isEmpty() && !expectedTags.contains(value.tag()))
      doThrow = true;

    if (doThrow) {
      throw DeserializationException(
          QByteArray("Failed to deserialze CBOR-value to type ") +
          QMetaTypeName(propertyType) +
...
QVariant
SerializerBasePrivate::deserializeJsonValue(int propertyType,
                                            const QCborValue &value) const {
  // perform strict validations
  if (validationFlags.testFlag(ValidationFlag::StrictBasicTypes)) {
    auto doThrow = false;
    ...
    if (doThrow) {
      throw DeserializationException(
          QByteArray("Failed to deserialze JSON-value to type ") +
          QMetaTypeName(propertyType) +
          QByteArray("because the given JSON-value failed strict validation"));
    }
  }

  // special case: QRegularExpression, is missing a converter (and cannot be
  // registered)
  if (propertyType == QMetaType::QRegularExpression && value.isString())
