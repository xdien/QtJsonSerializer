...
#else
  return QVariant{QMetaType(metaTypeId), nullptr}.canConvert(
             QMetaType(QMetaType::QVariantList)) &&
         SequentialWriter::canWrite(metaTypeId);
#endif
}
...
QList<QCborValue::Type> ListConverter::allowedCborTypes(int metaTypeId,
                                                        QCborTag tag) const {
  Q_UNUSED(metaTypeId)
  Q_UNUSED(tag)
...
QCborValue ListConverter::serialize(int propertyType,
  ...
  if (!value.canConvert(QMetaType(QMetaType::QVariantList))) {
    ...
    throw SerializationException(
        ...
        QByteArray(" cannot be processed via QSequentialIterable - make shure "
  ...
  for (const auto &element : value.value<QSequentialIterable>())
    array.append(helper()->serializeSubtype(
        info.type, element, "[" + QByteArray::number(index++) + "]"));
  if (info.isSet)
...
QVariant ListConverter::deserializeCbor(int propertyType,
  ...
  if (!writer) {
    throw DeserializationException(
        QByteArray("Given type ") + QMetaTypeName(propertyType) +
        QByteArray(
            " cannot be accessed via QSequentialWriter - make shure to "
            "register it via QJsonSerializerBase::registerListConverters or "
            "QJsonSerializerBase::registerSetConverters"));
  }
  ...
  for (auto element : array)
    writer->add(helper()->deserializeSubtype(
        info.type, element, parent, "[" + QByteArray::number(index++) + "]"));
  return list;
}
