...
QList<QCborValue::Type> MapConverter::allowedCborTypes(int metaTypeId,
                                                       QCborTag tag) const {
  Q_UNUSED(metaTypeId)
  Q_UNUSED(tag)
...
QCborValue MapConverter::serialize(int propertyType,
...
QVariant MapConverter::deserializeCbor(int propertyType,
  ...
  for (const auto entry : cborMap) {
    ...
    writer->add(helper()->deserializeSubtype(info.keyType, entry.first, parent,
                                             keyStr + ".key"),
