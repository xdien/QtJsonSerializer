...
QList<QCborTag> MultiMapConverter::allowedCborTags(int metaTypeId) const {
  Q_UNUSED(metaTypeId)
  return {NoTag, static_cast<QCborTag>(CborSerializer::MultiMap),
          static_cast<QCborTag>(CborSerializer::ExplicitMap)};
}

QList<QCborValue::Type>
MultiMapConverter::allowedCborTypes(int metaTypeId, QCborTag tag) const {
  Q_UNUSED(metaTypeId)
  Q_UNUSED(tag)
  return {QCborValue::Map, QCborValue::Array};
}

QCborValue MultiMapConverter::serialize(int propertyType,
  ...
  if (!value.canConvert(QMetaType(QMetaType::QVariantMap)) &&
    ...
    throw SerializationException(
        QByteArray("Given type ") + QMetaTypeName(propertyType) +
        QByteArray(" cannot be processed via QAssociativeIterable - make shure "
                   "to register the container type via "
                   "Q_DECLARE_ASSOCIATIVE_CONTAINER_METATYPE"));
  }

  // write from map to cbor
  const auto mapMode = helper()
                           ->getProperty("multiMapMode")
                           .value<SerializerBase::MultiMapMode>();
  ...
  case SerializerBase::MultiMapMode::DenseMap: {
    ...
    for (auto it = iterable.begin(), end = iterable.end(); it != end; ++it) {
      ...
      case QCborValue::Array: {
        ...
        mValueRef =
            QCborValue{}; // "clear" the array spot, reducing the ref cnt on
                          // mArray to 1, so stuff is added without copying
        mArray.append(helper()->serializeSubtype(info.valueType, it.value(),
                                                 keyStr + ".value"));
        mValueRef = mArray;
        break;
      }
      case QCborValue::Undefined:
        mValueRef = helper()->serializeSubtype(info.valueType, it.value(),
                                               keyStr + ".value");
        break;
      default: {
  ...
  case SerializerBase::MultiMapMode::List: {
    ...
    for (auto it = iterable.begin(), end = iterable.end(); it != end; ++it) {
      ...
      cborArray.append(QCborArray{
          helper()->serializeSubtype(info.keyType, it.key(), keyStr + ".key"),
          helper()->serializeSubtype(info.valueType, it.value(),
                                     keyStr + ".value")});
    }
...
QVariant MultiMapConverter::deserializeCbor(int propertyType,
  ...
  if (!writer) {
    throw DeserializationException(
        QByteArray("Given type ") + QMetaTypeName(propertyType) +
        QByteArray(
            " cannot be accessed via QAssociativeWriter - make shure to "
            "register it via QJsonSerializerBase::registerMapConverters"));
  }
  ...
  case QCborValue::Map: {
    for (const auto entry : cValue.toMap()) {
      const QByteArray keyStr =
          "[" + entry.first.toVariant().toString().toUtf8() + "]";
      const auto key = helper()->deserializeSubtype(info.keyType, entry.first,
                                                    parent, keyStr + ".key");
      if (entry.second.isArray()) {
        auto cnt = 0;
        for (const auto aValue : entry.second.toArray())
          writer->add(
              key, helper()->deserializeSubtype(
                       info.valueType, aValue, parent,
                       keyStr + ".value[" + QByteArray::number(cnt++) + "]"));
      } else
        writer->add(key,
                    helper()->deserializeSubtype(info.valueType, entry.second,
                                                 parent, keyStr + ".value"));
    }
    break;
  }
  case QCborValue::Array: {
    for (const auto aValue : cValue.toArray()) {
      ...
      if (vPair.size() != 2)
        throw DeserializationException(
            "CBOR/JSON array must have exactly 2 elements to be read as a "
            "value of a multi map");
      const QByteArray keyStr =
          "[" + vPair[0].toVariant().toString().toUtf8() + "]";
      writer->add(helper()->deserializeSubtype(info.keyType, vPair[0], parent,
                                               keyStr + ".key"),
                  helper()->deserializeSubtype(info.valueType, vPair[1], parent,
                                               keyStr + ".value"));
    }
    break;
  }
  default:
    throw DeserializationException("Unsupported CBOR/JSON-Type: " +
